...
#include <pairs_msgs/PoseWithCovarianceArrayStamped.h>
...
struct display_object
{
  ...
  boost::shared_ptr<pairs_rviz_plugins::covariance::Visual> covariance_;
};

class DisplaySelectionHandler;

class Display : public rviz::MessageFilterDisplay<pairs_msgs::PoseWithCovarianceArrayStamped> {
  Q_OBJECT
public:
  enum Shape
  {
...
private:
  ...
  virtual void processMessage(const pairs_msgs::PoseWithCovarianceArrayStamped::ConstPtr& message);
