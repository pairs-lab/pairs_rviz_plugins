#ifndef PAIRS_STATUS_DISPLAY_H
#define PAIRS_STATUS_DISPLAY_H
...
#include <pairs_msgs/NodeCpuLoad.h>
#include <pairs_msgs/CustomTopic.h>
...
private:
  ...
  // Subscriber callback
  void uavStatusCb(const pairs_msgs::UavStatusConstPtr& msg);

  // New message processing methods
  void processTopLine(const pairs_msgs::UavStatusConstPtr& msg);
  void processControlManager(const pairs_msgs::UavStatusConstPtr& msg);
  void processOdometry(const pairs_msgs::UavStatusConstPtr& msg);
  void processGeneralInfo(const pairs_msgs::UavStatusConstPtr& msg);
  void processHwApiState(const pairs_msgs::UavStatusConstPtr& msg);
  void processCustomTopics(const pairs_msgs::UavStatusConstPtr& msg);
  void processCustomStrings(const pairs_msgs::UavStatusConstPtr& msg);
  void processNodeStats(const pairs_msgs::UavStatusConstPtr& msg);
  ...
  // Custom topics
  std::vector<pairs_msgs::CustomTopic> custom_topic_vec;
  ...
  // Node stats
  pairs_msgs::NodeCpuLoad node_cpu_load_vec;
