...
void StatusDisplay::onInitialize() {
  ...
  uav_status_sub =
      nh.subscribe(uav_name_property->getStdString() + "/pairs_uav_status/uav_status", 10, &StatusDisplay::uavStatusCb, this, ros::TransportHints().tcpNoDelay());
...
void StatusDisplay::uavStatusCb(const pairs_msgs::UavStatusConstPtr& msg) {
  processTopLine(msg);
...
void StatusDisplay::processTopLine(const pairs_msgs::UavStatusConstPtr& msg) {
...
void StatusDisplay::processControlManager(const pairs_msgs::UavStatusConstPtr& msg) {
...
void StatusDisplay::processOdometry(const pairs_msgs::UavStatusConstPtr& msg) {
...
void StatusDisplay::processGeneralInfo(const pairs_msgs::UavStatusConstPtr& msg) {
...
void StatusDisplay::processHwApiState(const pairs_msgs::UavStatusConstPtr& msg) {
...
void StatusDisplay::processCustomTopics(const pairs_msgs::UavStatusConstPtr& msg) {
  std::vector<pairs_msgs::CustomTopic> new_custom_topic_vec = msg->custom_topics;

  topics_update_required |= compareAndUpdate(new_custom_topic_vec, custom_topic_vec);
}

void StatusDisplay::processCustomStrings(const pairs_msgs::UavStatusConstPtr& msg) {
...
void StatusDisplay::processNodeStats(const pairs_msgs::UavStatusConstPtr& msg) {
  pairs_msgs::NodeCpuLoad new_node_cpu_load_vec = msg->node_cpu_loads;
...
PLUGINLIB_EXPORT_CLASS(pairs_rviz_plugins::StatusDisplay, rviz::Display)
