...
namespace pairs_rviz_plugins
{
  ...
  namespace bumper
  {
    ...
    void Display::processMessage(const pairs_msgs::ObstacleSectors::ConstPtr& msg)
    {
      ...
      for (const auto cur_len : msg->sectors)
      {
        ...
        {
          ROS_DEBUG("[Visual]: Invalid obstacle distance encountered in pairs_msgs::ObstacleSectors message: %.2f, skipping message", cur_len);
          return;
        }
      }
...
PLUGINLIB_EXPORT_CLASS(pairs_rviz_plugins::bumper::Display, rviz::Display)
