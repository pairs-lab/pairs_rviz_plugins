...
void PoseEstimate::onInit() {
  ...
  pairs_lib::ParamLoader param_loader(nh_, "PoseEstimate");
  ...
  profiler_ = pairs_lib::Profiler(nh_, "PoseEstimate", _profiler_enabled_);
...
void PoseEstimate::callbackRvizPoseEstimate(const geometry_msgs::PoseWithCovarianceStampedConstPtr &msg) {
  ...
  pairs_lib::Routine profiler_routine = profiler_.createRoutine("callbackRvizPoseEstimate");
  ...
  // Prepare transformer
  ros::ServiceClient         srv_client_transform_pose = nh_.serviceClient<pairs_msgs::TransformPoseSrv>("/" + uav_name + "/control_manager/transform_pose");
  pairs_msgs::TransformPoseSrv transform_pose_srv;
  ...
  try {
    heading_tmp = pairs_lib::AttitudeConverter(orientation_tmp).getHeading();
  }
...
void PoseEstimate::timerPub(const ros::TimerEvent &event) {
  ...
  pairs_lib::Routine profiler_routine = profiler_.createRoutine("timerPub", _pub_rate_, 0.01, event);
...
PLUGINLIB_EXPORT_CLASS(pairs_rviz_plugins::rviz_interface::PoseEstimate, nodelet::Nodelet)
