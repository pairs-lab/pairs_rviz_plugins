...
/* DisplaySelectionHandler and some functions from 
        https://github.com/ctu-pairs/pairs_rviz_plugins/blob/master/src/pose_with_covariance_array/display.cpp/ */
...
public:
  ...
  void setMessage(const pairs_msgs::TrackArrayStampedConstPtr& message) {
    message_ = message;
...
private:
  ...
  pairs_msgs::TrackArrayStampedConstPtr message_;
  Display* display_;
};
...
void Display::processMessage(const pairs_msgs::TrackArrayStamped::ConstPtr& message) {
  disp_data_.clear();
...
PLUGINLIB_EXPORT_CLASS(pairs_rviz_plugins::track_array::Display, rviz::Display)