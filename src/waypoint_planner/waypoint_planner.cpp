...
WaypointPlanner::WaypointPlanner() {
  shortcut_key_ = 'w';
  transformer   = pairs_lib::Transformer(node_handler);
...
void WaypointPlanner::update_topic() {
  client =
      node_handler.serviceClient<pairs_msgs::PathSrv>(std::string("/") + drone_name_property->getStdString() + std::string("/") + topic_property->getStdString());
...
void WaypointPlanner::processLoop() {
  ...
  client = node_handler.serviceClient<pairs_msgs::Vec4>(std::string("/") + drone_name_property->getStdString() + std::string("/control_manager/goto_fcu"));
  ...
  ;
  while (true) {
    const auto& info_msg = ros::topic::waitForMessage<pairs_msgs::ControlManagerDiagnostics>(topic, node_handler, ros::Duration(1.0));
...
std::vector<pairs_msgs::Reference> WaypointPlanner::generateReferences(std::string current_frame, int num) {
  ...
  std::vector<pairs_msgs::Reference> res(num);
  ...
  if (!tf) {
    ...
    return std::vector<pairs_msgs::Reference>{};
  }
  ...
  for (int i = 0; i < positions.size() && i < num; i++) {
    ...
    pose                  = point_transformed.value();
    const double rotation = pairs_lib::AttitudeConverter(pose.orientation).getHeading();
...
PLUGINLIB_EXPORT_CLASS(pairs_rviz_plugins::WaypointPlanner, rviz::Tool)
