...
public:
      ...
      if (display_->pose_valid_) {
        ...
        if (display_->covariance_property_->getBool()) {
          ...
          if (display_->covariance_property_->getOrientationBool()) {
            aabbs.push_back(
                cur_pose.covariance_->getOrientationShape(pairs_rviz_plugins::covariance::Visual::kRoll)->getEntity()->getWorldBoundingBox());
            aabbs.push_back(
                cur_pose.covariance_->getOrientationShape(pairs_rviz_plugins::covariance::Visual::kPitch)->getEntity()->getWorldBoundingBox());
            aabbs.push_back(
                cur_pose.covariance_->getOrientationShape(pairs_rviz_plugins::covariance::Visual::kYaw)->getEntity()->getWorldBoundingBox());
          }
        }
      }
    }
  ...
  void setMessage(const pairs_msgs::PoseWithCovarianceArrayStampedConstPtr& message)
  {
    message_ = message;
...
private:
  ...
  pairs_msgs::PoseWithCovarianceArrayStampedConstPtr message_;
  Display* display_;
};
...
void Display::processMessage(const pairs_msgs::PoseWithCovarianceArrayStamped::ConstPtr& message) {
  disp_data.clear();
...
PLUGINLIB_EXPORT_CLASS(pairs_rviz_plugins::pose_with_covariance_array::Display, rviz::Display)
